...
function UpdateEvent() {
  ...
  const getcord = async () => {
    ...
    setcord(json);
    if (json) {
      setevent_name(json[0].event_name);
      setdept_name(json[0].dept_name);
      setdate(json[0].date);
      settime(json[0].time);
      setevent_desc(json[0].event_desc);
      setfee(json[0].fee);
      setteam(json[0].team);
      setfaculty_cordinate(json[0].faculty_cordinate);
      setfaculty_number(json[0].faculty_number);
      setstd_cordinator(json[0].std_cordinator);
      setstd_contact(json[0].std_contact);
      setupi(json[0].upi);
    }
  };
  useEffect(() => {
